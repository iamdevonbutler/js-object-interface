 ...
 */
function applyInterface(src, methods = null) {
  var obj;
  ...
  obj.map = (cb, wrapResult = false) => {
    ...
    if (async) {
        ...
        Promise.all(promises)
          .then(values => {
            ...
            if (Object.keys(obj1).length) {
              resolve(wrapResult ? applyInterface(obj1, obj._methods) : obj1);
            }
            else {
              resolve(null);
            }
      ...
      keys.forEach((key, i) => obj1[key] = values[i]);
      if (Object.keys(obj1).length) {
        return wrapResult ? applyInterface(obj1, obj._methods) : obj1;
      }
      else {
        return null;
      }
  ...
  obj.clone = (wrap = false) => {
    var result = wrap ? applyInterface(objectAssign(obj.src), obj._methods) : objectAssign(obj.src);
    return result;
  };

  /**
   * Returns a new Object. New obj inherits properties from the original and deeply assign new properties.
   * To update the src Object - obj.set(obj.assign(newObj))
   * @param  {Object} obj e.g. obj.assign(obj, obj1, ...)
   * @return {Object}
   */
  obj.assign = (...args) => {
    args = args.filter(Boolean).map(arg => arg._isObjectInterface ? arg.src : arg);
    var result = objectAssign(obj.src, ...args);
    return result;
  };
   ...
   */
  obj._wrap = (obj1) => {
    var result = isObject(obj1) ? applyInterface(obj1, obj._methods) : undefined;
    return result;
  };

  /**
   * Add custom methods to your object.
   */
  if (methods) {
    obj._methods = methods;
    methods = applyInterface(methods);
    methods.forEach((method, name) => {
      obj[name] = method.bind(obj.src);
    });
  }
  else {
    obj._methods = undefined;
  }
