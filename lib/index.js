...
function applyInterface(src, clone = true) {
   ...
   * $value wraps values of type Object in an interface itself - to, for instance,
   * execute a nested map. If the value is not an object,
   * $value is undefined.
   * @param  {Function} cb params: value, key, $value
   * @param  {Boolean} [wrapResult=true]
   * @return {Object|Promise|$Object} returns Promise if async, a wraped result, or unwrapped result.
   */
  obj.map = (cb, wrapResult = true) => {
   ...
   * returns a truthy value. We return the key, value, and $value, encapsulated
  ...
  obj.find = (cb) => {
    ...
    return result ? {key: result, value: obj.src[result], $value: obj._wrap(obj.src[result])} : undefined;
  };
